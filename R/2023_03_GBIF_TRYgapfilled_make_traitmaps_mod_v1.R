...
file <- paste0("/net/home/tkattenborn/data_global_maps/gbif_sample_all_cells_hex9_", paste0(pft, collapse = "_"), ".csv") #random sample max. 10000 obs, hex9, GBIF, TRY-gapfilled
...
for (j in rasters) {
  ...
  for (i in loop.vector) { # Loop over loop.vector
    ...
    r1 <- rasterize(xy_sub, j, vals, fun=function(x,...)c(length(x),mean(x),median(x),sd(x), quantile(x, probs = c(0.05)), quantile(x, probs = c(0.95))))
    ...
    filename1 = paste("/net/home/tkattenborn/data_global_maps/trait_maps_gbif/", paste0(pft, collapse = "_"),"/", folder_name[index],  "/GBIF_TRYgapfilled_", name1, "_", folder_name[index], ".gri", sep="")
    print(filename1)
